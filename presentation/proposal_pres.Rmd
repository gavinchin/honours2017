...
With this project, we want to improve the way we visualise Melbourne's pedestrian activity, and use a predicitive model to get a better understanding of how Melbourne operates

```{r, echo = FALSE}
ggmap(melb) + geom_point(aes(x = Lon, y = Lat, size = Hourly_Counts), alpha = 0.02, data = filter(ped_df, Year == "2016", Month == "March", Time == "9"), colour = "violetred") + scale_size_area(max_size = 10) + guides(colour = guide_legend(override.aes = list(alpha = 1)))
```
