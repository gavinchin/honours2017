...
- inputs required: 
  ...
  * `threshold` and `maxlength` are tuning parameters
...
```{r, echo = FALSE, fig.retina = 6, message=FALSE, warning=F, fig.width = 10, fig.height = 2.5}
melb_cent_mon9 <- dfa2 %>%
  filter(Day == "Monday",
         Time == 9) %>%
         .$`Melbourne Central`
...
- can use clustering methods to investigate similarities between locations
  * potentially use different models based on cluster membership
  * eg. train stations, parks and recreation, shopping
#### Objective: On the fly predictions of pedestrian traffic
- requires efficient data storage

---
# Future steps for final paper
- perform simulations to test imputation algorithm
...
```{r, cache = TRUE, fig.retina = 4, echo = FALSE, message = FALSE, fig.align= 'center', fig.width=5, fig.height=3.5, warning=F}
...
---
# Summary
- investigated issues with the Melbourne CBD pedestrian data
- proposed algorithm for imputation of missing values in pedestrian data using split approach based on missing values proportion
- 
...
- An Introduction to Statistical Learning - _G. James, D. Witten, T. Hastie and R. Tibshirani_
- 
...
GitHub repository for this project: https://github.com/gavinchin/honours2017
