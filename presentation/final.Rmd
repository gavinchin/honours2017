...
```{r, fig.retina = 3, echo = FALSE, message = FALSE, fig.height = 4, fig.align = 'center'}
loc <- loc %>% mutate(cl = ifelse((loc$ids %in% small_miss_sensors),
                                  "Small", "Large"))
melbbw <- get_map(location=c(mean(range(ped_loc$Longitude)),
                            mean(range(ped_loc$Latitude))),
                            zoom=14, source = "stamen", maptype = "toner")

ggmap(melbbw) + geom_point(aes(x = lons, y = lats, colour = cl),
...
## Step 2: Fit models with "small" proportion of missing values
