...
$$E \left[\text{Hourly_Counts} \vert \text{Sensor}, \text{HDay}, \text{Time} \right] = \exp (\mu_{\text{Sensor}, \text{HDay}, \text{Time}}) $$
...
cl <- makeCluster(2)
registerDoSNOW(cl)
models <- foreach(i = unique(ped_data$Sensor_ID)) %dopar% {
...
run_lengths <- foreach(i = unique(ped_data$Sensor)) %do%
  {
    dfa[is.na(dfa[, i]), i] <- 0
...
$$E \left[\text{Hourly_Counts} \vert  \text{Sensor}, \text{Month}, \text{DayType}, \text{Time} \right] = \exp (\mu_{\text{Sensor}, \text{Month}, \text{DayType}, \text{Time}}) $$
...
models <- foreach(i = small_miss_sensors) %dopar%
{
  p$tick()$print()
...
for (i in large_miss_sensors){
  ...
  closest_sensor <- sensor_dists %>% sort %>%
                    head(2) %>% names
  close_df_scale <- dfa[, closest_sensor[1]] %>% unlist %>% scale
  close_df_scale2 <- dfa[, closest_sensor[2]] %>% unlist %>% scale

  sensor_df <- dfa[, i] %>% unlist
  model <- glm(sensor_df ~ Time*close_df_scale + Time*close_df_scale2, data = dfa, family = quasipoisson())
...
$$E \left[ \text{Hourly_Counts}_{\text{sensor}} \vert \text{Time}, \text{Hourly_Counts}_{\text{closest sensors}} \right] = \mu_{\text{Time}, \text{Hourly_Counts}_{\text{closest sensors}}}$$
$$\text{where } \quad  \mu_{\text{Time}, \text{Hourly_Counts}_{\text{closest sensors}}} \sim \text{Time} \times \text{Hourly_Counts}_{\text{closest sensor 1}} + \text{Time} \times \text{Hourly_Counts}_{\text{closest sensor 2}}$$
