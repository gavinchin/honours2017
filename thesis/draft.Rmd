...
```{r, fig.width=4, fig.height=4, fig.align='center', fig.cap = "Map showing the algorithm selecting Flinders St Station Underpass and Sandridge Bridge as neighbours to be used to predict the pedestrian counts at Southbank"}
...
Again, we start by comparing the fit at a sensor location with a small proportion of missing values, Vic Arts Centre:
...
```{r, fig.height=3.5, fig.width=9, fig.cap = paste("Distribution of out-of-sample (2017 data) MARE by imputation model, excluding Spencer St-Collins St (North)/(South) sensors")}

grid.arrange(p1, p2, p3, nrow = 1)

```

Spencer St-Collins St (North)/(South) was omitted from the the plots above as the out-of-sample MARE is extremely high at `r round(MARE_ts["Spencer St-Collins St (North)"]*100, 2)`% and `r round(MARE_ts["Spencer St-Collins St (South)"]*100, 2)`% respectively. 

As these sensor locations were both classified as locations with a large proportion of missing values, it would appear that the relationship between these locations and their neighbouring sensors has changed. This is because the in-smaple MARE at these sensor locations were quite acceptable at `r round(MARE_tr["Spencer St-Collins St (North)"]*100, 2)`% and `r round(MARE_tr["Spencer St-Collins St (South)"]*100, 2)`% respectively. 
...
Another way we can perform cross-validation is to remove data at a sensor, and evaulate the predictions made. This will allow us to evaluate the imputation model's performance within sample, which is the primary concern with the imputation process.  
...
```{r, fig.width = 8, fig.height=2, fig.cap = "Plot of simulated missingness method 2 at Bourke Street Mall (South)"}
