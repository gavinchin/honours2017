...
Using a GLM predicts well compared to estimating with other models such as AR(p) or other time series models when considering the tradeoff between model complexity and prediction accuracy.  
...
```{r, fig.height=4, fig.width=8, fig.cap=paste("Plot of the estimated coefficients of the GLM at each sensor for each variable. Variables (x-axis) is ordered by the scagnostic measure, Outlying.")}
...
library(GGally)
library(scagnostics)
ggparcoord(dfcoeff[, -1],
           order = "Outlying", alphaLines = 0.3)
```
