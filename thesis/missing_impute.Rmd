...
With the function `model.matrix()`, `R` will generate a matrix, $X$, with the factor levels converted into dummy variables for use with the GLM. Then, we can obtain our estimates of $\text{Hourly_Counts}_ij$, $\widehat{\text{Hourly_Counts}_i} = \exp \left( X \hat{\beta_j} \right)$ where $\hat{\beta_j}$ is a column vector of the GLM estimated coefficients at the $j^{th}$ sensor location.

# Goodness of Fit
### Visually
```{r}
ggplot(filter(test_fitted, Date > "2017-07-23")) + geom_line(aes(x = Date_Time, y = Hourly_Counts)) +
                         geom_line(aes(x = Date_Time, y = Fitted), colour = "violetred2", alpha = 0.75) +
                         facet_wrap(~ Sensor, scales = "free_y", ncol = 5) + theme_minimal() + 
...
Using RMSE (root mean square error) as a goodness of fit measure, we find that the RMSE of the prediction represented in the plot above is `r col_pl_test_rmse`. 
...
```{r}
ggplot(filter(ped_data, Sensor == "Collins Place (South)")) + geom_smooth(aes(x = as.integer(Time), y = Hourly_Counts, colour = Day))
```
