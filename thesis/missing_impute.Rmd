...
```{r pubhdays, message=F, warning=F}
...
```
Compacts the `HDay` factor into 6 levels, rather than 8 levels, by coding Tuesday, Wednesday and Thursday as factor `"Midweek"`. This is because at most locations, these days have similar patterns.

## GLM model for imputing missings
```{r glm_models, results='hide'}
...
With the function `model.matrix()`, `R` will generate a matrix, $X$, with the factor levels converted into dummy variables for use with the GLM. Then, we can obtain our estimates of $\text{Hourly_Counts}_{ij}$, $\widehat{\text{Hourly_Counts}_i} = \exp \left( X \hat{\beta_j} \right)$ where $\hat{\beta_j}$ is a column vector of the GLM estimated coefficients at the $j^{th}$ sensor location.

# Goodness of Fit
### Visually
```{r, fig.align= 'center', fig.height=24, fig.retina=2, dev='png', dev.args=list(type="cairo"), dpi = 128}
ggplot(filter(test_fitted, Date > "2017-07-23")) +
  geom_line(aes(x = Date_Time, y = Hourly_Counts), size = 2) +
  geom_line(aes(x = Date_Time, y = Fitted),
            colour = "violetred2", alpha = 0.75, size = 2) +
...
```{r, fig.align= 'center', fig.retina=4, dev='png', dev.args=list(type="cairo"), dpi = 128}
ggplot(test_fitted %>%
...
```{r}
col_pl_test_mse <- test_fitted %>% filter(Date > "2017-07-23", Sensor == "Collins Place (North)") %>%
               mutate(residsq = (Fitted - Hourly_Counts)^2) %>% .$residsq %>% mean()
col_pl_test_rmse <- sqrt(col_pl_test_mse)
```

Using RMSE (root mean square error) as a goodness of fit measure, we find that the RMSE of the prediction represented in the plot above is `r round(col_pl_test_rmse, 4)`. 

```{r, fig.align= 'center'}
ggplot(filter(test_fitted, Sensor_ID == "Melbourne Central")) +
...
```{r, message = F, warning = F, fig.align= 'center', dev='png', dev.args=list(type="cairo"), dpi = 128}
